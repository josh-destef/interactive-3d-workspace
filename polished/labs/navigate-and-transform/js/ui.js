...
function showReplay() { tog('btn-replay', true); }
function hideReplay() { tog('btn-replay', false); }
...
/* ── panel position helper ──
   ...
   not draggable, so this is a no-op kept only so existing call sites stay valid. */

export function dockPanel() { /* panel is fixed-docked - nothing to do */ }

/* Briefly animate the Reset View / Reset Gizmobot buttons to draw attention when a
