...
═══════════════════════════════════════════════ */
import { clamp, easeInOut } from './utils.js?v=selector5';
...
export function tickAnims(dt) {
    for (let i = runningAnims.length - 1; i >= 0; i--) {
        ...
        // First tick delivers exactly t = 0 so the `if (t === 0) …` setup gates
        // inside the beat demos (show cursor, press down) actually run.
        if (!a.started) {
            a.started = true;
            a.fn(0);
            continue;
