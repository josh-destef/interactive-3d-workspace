...
export const hierarchySection = {
  id: 'hierarchy', title: 'Hierarchy', defaultExpanded: true,
  supports: () => true,
  render({ project, entity, capabilities = {} }) {
    ...
    parentLabel.hidden = capabilities.actions?.reparent === false;
    ...
    group.hidden = capabilities.actions?.group === false;
    ...
    ungroup.hidden = capabilities.actions?.ungroup === false;
    ...
    function sync(next) {
      ...
      if (document.activeElement !== select) {
        ...
        const sceneOption = el('option', '', 'No parent'); sceneOption.value = ''; select.append(sceneOption);
        project.entities.filter(candidate => candidate.id !== next.id && !descendants.has(candidate.id)).forEach(candidate => {
